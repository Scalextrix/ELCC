...
def databasecreate():
	...
	conn.commit()
	c.execute('''CREATE TABLE IF NOT EXISTS SYSTEMDETAILS (dataloggerid BLOB, systemid TEXT, userid TEXT, inverterip TEXT, panelid TEXT, tilt TEXT, azimuth TEXT, inverterid TEXT, datalogger TEXT, pkwatt TEXT, lat TEXT, lon TEXT, msg TEXT, slrsigaddr BLOB, apikey TEXT)''')
	c.execute("INSERT OR REPLACE INTO SYSTEMDETAILS VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);", (datalogger_id, system_id, user_id, inverter_ip, solar_panel, tilt, azimuth, solar_inverter, d_logger_type, peak_watt, latitude, longitude, message, solarcoin_sig_address, api_key))
	conn.commit()
	conn.close()
...
def inverterqueryincrement():
	...
	SolarEdge API usage limit (300 per day) """
...
if os.path.isfile("APIlansig.db"):
	...
	if system_update_chooser == 'Y':
		...
		user_id = ""
		inverter_ip = ""
		inverter_serial_no = raw_input ("What is your SolarEdge Inverter Serial Number: ")
		datalogger_id = hashlib.sha1(inverter_serial_no+solar_panel+str(tilt)+str(azimuth)+solar_inverter+d_logger_type+str(peak_watt)+latitude+longitude).hexdigest()
		databasecreate()
		comm_creds = retrievecommoncredentials()
		print 'New UID: {}'.format(comm_creds['datalogger_id'])
